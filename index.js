(async()=>{
  ...
  let preverr = null
  let repeaterrc = 1
  require('./databases/database transfer.js')
  require('dotenv').config();
  let process = require('process');
  process.on('uncaughtException', function (err) {
    errcount++
    if (String(err) == preverr) {
      repeaterrc++
      console.log('x' + repeaterrc, errcount)
      preverr = String(err)
      return
    }
    repeaterrc = 1
  ...
  let { MessageButton, MessageActionRow, Permissions, MessageSelectMenu } = require("discord.js")
  ...
  var command, prefix, arguments2, user_appeal, appeal_step, temp, send, number, topic, menu, t, channel, message, role_id_s, reson, progress, db, out, lb, list, j, i, percent, object, lb_page, object_two;
  const yes = await jimp.read('bases/yes.png')
  const no = await jimp.read('bases/no.png')
  const shipON = '<:starton:994026992385130556><:middleon:994026994931093605><:middleon:994026994931093605><:middleon:994026994931093605><:middleon:994026994931093605><:middleon:994026994931093605><:middleon:994026994931093605><:endon:994028214806650950>'
  const shipOFF = '<:startoff:994028215842639964><:middleoff:994026994931093605><:middleoff:994026994931093605><:middleoff:994026994931093605><:middleoff:994026994931093605><:middleoff:994026994931093605><:middleoff:994026994931093605><:endoff:994026988643831860>'
  const segments = 10
  ...
  const mdb = database.collection('mongoDB');
  ...
  s4d.client.on('messageCreate', async (s4dmessage) => {
    if (((s4dmessage.content) || '').startsWith(prefix || '')) {
      ...
      try{
          ...
          break;
          case 'ship': {
            let user = s4dmessage.member.user
            let ship = s4dmessage.guild.members.cache.random().user;
            if (s4dmessage.mentions.users.size > 1) {
              user = s4dmessage.mentions.users.at(0);
              ship = s4dmessage.mentions.users.at(1);
            } else if (s4dmessage.mentions.users.size > 0) {
              user = s4dmessage.mentions.users.at(0);
              ship = s4dmessage.guild.members.cache.random().user;
            }
            const userPFP = await jimp.read(user.displayAvatarURL({ format: "png" }))
            const shipPFP = await jimp.read(ship.displayAvatarURL({ format: "png" }))
            shipPFP.resize(128, 128, jimp.RESIZE_BEZIER)
            userPFP.resize(128, 128, jimp.RESIZE_BEZIER)
            progress = Math.min(Math.max(Number(subsequenceFromStartLast(ship.id, 16)) - Number(subsequenceFromStartLast(user.id, 17)) / 4, 0), segments * segments) / segments;
            s4dmessage.channel.send('finding someone');
            out = [
              shipON.slice(0, progress - 1),
              shipOFF.slice(progress, segments - progress)
            ].join('');
            percent = Math.round((progress / segments) * 100);
            var embed = new Discord.MessageEmbed();
              embed.setColor('#009900');
              embed.setTitle([user.username, ' X ', ship.username].join(''))
              embed.setDescription([
                '**',
                percent,
                '%** ',
                out,
                percent < 25 ? ' worse than average' : '',
                percent < 50 ? ' not very good' : '',
                percent > 50 ? ' good' : '',
                percent > 75 ? ' great!' : ''
              ].join(''))
              await jimp.read('bases/base.png', async (err, image) => {
                ...
                image.resize(image.bitmap.width * 2 + 40, image.bitmap.height)
                image.blit(userPFP, 0, 0)
                image.blit(shipPFP, temp + 40, 0)
                image.blit(percent >= 50 ? yes : no, temp - 16, 32)
                image.getBuffer(jimp.MIME_PNG, function (err, buffer){
                  const attachment = new Discord.MessageAttachment(buffer, 'result.png')
                  s4d.client.channels.cache.get('994108471958044754').send({
                    files: [attachment]
                  })
                });
              });
              await s4d.client.channels.cache.get('994108471958044754').messages.fetch({ limit: 1 }).then(function (last_messages_in_channel) {
                embed.setImage(last_messages_in_channel.at(0).attachments.at(0).url);
              });
              embed.setTimestamp(new Date());
              s4dmessage.channel.send({embeds: [embed]});
          }
  ...
  /* s4d.client.on('messageCreate', async (s4dmessage) => {
    if (s4dmessage.type == "REPLY") {
      if (s4dmessage.mentions.repliedUser.id == '882556092893245501') {
        if (s4dmessage.content.startsWith('stfu') || s4dmessage.content.startsWith('shut')) {
          s4dmessage.reply({
            content: 'no as i was not programed to be able to', 
            allowedMentions: {
              repliedUser: false
            }
          });
        ...
        https.get([
          ...
          s4dmessage.content,
          '&owner=godslayerakp&botname=arcade tools'
        ].join(''), async resp => {
          let data2 = "";
          resp.on("data", async chunk => {
            data2 += chunk;
    ...
    if (!s4dmessage.author.bot) {
      if (!(await leveling.get('no-message')).includes(s4dmessage.channel.id)) {
        if (!leveling.has(s4dmessage.author.id)) {
          temp = {
            'message': s4dmessage.content,
            'timestamp': s4dmessage.createdTimestamp,
            'level-mult': 7,
            'exp': 0,
            'level': 1
          };
          leveling.set(s4dmessage.author.id, temp);
          return
        }
        temp = await leveling.get(s4dmessage.author.id);
        if (temp['message'] == s4dmessage.content) {
          return
        }
        temp['message'] = s4dmessage.content
        temp['timestamp'] = s4dmessage.createdTimestamp
        temp['exp'] = temp['exp'] + 1
        if (temp['exp'] > Math.pow(2, temp['level-mult'])) {
          s4dmessage.channel.send({
            content: [
              'congrats ',
          ...
          temp['level-mult'] = temp['level-mult'] + 1
          temp['level'] = temp['level'] + 1
          temp['exp'] = 0
        }
        leveling.set(s4dmessage.author.id, temp);
        roles = leveling.get('roles');
        if (roles[temp['level']] != null) {
          s4dmessage.member.roles.add(s4dmessage.guild.roles.cache.get(roles[temp['level']]));
        }
      }
    }
